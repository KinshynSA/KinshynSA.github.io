...
window.onload = ()=>{
	document.addEventListener('click', function(event){
		...
		if(event.target.closest('.description_butt-guest')) addTicket();		
		...
		if(event.target.closest('.main-button-done')){
...
function checkPform(e){
	...
	if(result){
		...
		addTicket(values);
		document.querySelector('.description_alert').classList.remove('hidden');
		document.querySelector('.main-buttons').classList.remove('hidden');
		document.querySelector('.description_butt-client').classList.add('hidden');
		document.querySelector('.description_butt-guest').classList.remove('hidden');
...
function changePrice(operator=true){
	...
	if(operator){
		t.textContent = +t.textContent + +t.dataset.price;
	} else {
		t.textContent = +t.textContent - +t.dataset.price;
	}
